...
public class CorridorPathRework extends ArrayContainer<CorridorStepNew> {
    ...
    public Array<Rectangle> compute(Vector2 start) {
        ...
        CorridorPath.SubPath subPath1 = new CorridorPath.SubPath(posPathOne.addTo(start.cpy(), firstStep.getWalkSize() / 2 + firstStep.getWallSize() / 2), posPathOne);
        CorridorPath.SubPath subPath2 = new CorridorPath.SubPath(postPathTwo.addTo(start.cpy(), firstStep.getWalkSize() / 2+ firstStep.getWallSize() / 2), postPathTwo);
        CorridorPath.Computer computer = new CorridorPath.Computer(start.cpy(), subPath1, subPath2);
    ...
    private class Computer {
        Vector2 middle;
        CorridorPath.SubPath path1;
        CorridorPath.SubPath path2;

        CorridorPath.SubPath pathOppositeLast;

        public Computer(Vector2 middle, CorridorPath.SubPath path1, CorridorPath.SubPath path2) {
            this.middle = middle;
            this.path1 = path1;
            this.path2 = path2;
        }
        ...
        public void add(CorridorPathStep step, Direction nextDir) {
            ...
            if (nextDir != null && currentDir != nextDir) {
                CorridorPath.SubPath pathOpposite = getOpposite(nextDir);
                CorridorPath.SubPath other = getOther(nextDir);
        ...
        private CorridorPath.SubPath getOpposite(Direction nextDir) {
            return path1.dirFromPos.getReverse() == nextDir ? path1 : path2;
        }

        private CorridorPath.SubPath getOther(Direction nextDir) {
